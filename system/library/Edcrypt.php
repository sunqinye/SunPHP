...
class Edcrypt{
	...
	public function encryptPassword($password){
		return md5(md5(md5($password.$this->crypt_key)));
	}

	public function encrypt($data){
		return $this->authcode($data, 'ENCODE', $this->crypt_key);
	}

	public function decrypt($data){
		return $this->authcode($data, 'DECODE', $this->crypt_key);
	}

	private function authcode($string, $operation, $key, $expiry = 0) {
		$ckey_length = 4;
		$key = md5($key);
